...
Revision ID: 08622f7198ff
Revises: 9b6ff45d5de5
Create Date: 2024-04-19 16:38:15.439630
...
revision = '08622f7198ff'
down_revision = '9b6ff45d5de5'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.add_column('candies', sa.Column('owner', sa.String(length=100), server_default='teacher', nullable=False))
    op.alter_column('candies', 'state',
               existing_type=sa.VARCHAR(length=20),
               nullable=False,
               existing_server_default=sa.text("'full'::character varying"))
    op.drop_column('candies', 'price')
    op.drop_column('candies', 'kid')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('candies', sa.Column('kid', sa.VARCHAR(length=100), autoincrement=False, nullable=False))
    op.add_column('candies', sa.Column('price', sa.INTEGER(), autoincrement=False, nullable=True))
    op.alter_column('candies', 'state',
               existing_type=sa.VARCHAR(length=20),
               nullable=True,
               existing_server_default=sa.text("'full'::character varying"))
    op.drop_column('candies', 'owner')
    op.create_table('fridgeo_receipt',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=0, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    sa.Column('title', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('receipt', sa.TEXT(), autoincrement=False, nullable=False),
    sa.Column('slug', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('time_create', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('is_published', sa.BOOLEAN(), autoincrement=False, nullable=False),
    sa.Column('cat_id', sa.BIGINT(), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['cat_id'], ['fridgeo_category.id'], name='fridgeo_receipt_cat_id_3eab76a7_fk_fridgeo_category_id', initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name='fridgeo_receipt_pkey'),
    sa.UniqueConstraint('slug', name='fridgeo_receipt_slug_8ba851a0_uniq')
    )
    op.create_index('fridgeo_receipt_slug_8ba851a0_like', 'fridgeo_receipt', ['slug'], unique=False)
    op.create_index('fridgeo_receipt_cat_id_3eab76a7', 'fridgeo_receipt', ['cat_id'], unique=False)
    op.create_index('fridgeo_rec_time_cr_f3c21c_idx', 'fridgeo_receipt', [sa.text('time_create DESC')], unique=False)
    ...
    sa.PrimaryKeyConstraint('id', name='fridgeo_user_pkey')
    )
    ...
    op.create_table('fridgeo_category',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    sa.Column('name', sa.VARCHAR(length=100), autoincrement=False, nullable=False),
    sa.Column('slug', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.PrimaryKeyConstraint('id', name='fridgeo_category_pkey'),
    sa.UniqueConstraint('slug', name='fridgeo_category_slug_key')
    )
    op.create_index('fridgeo_category_slug_c91f4b2b_like', 'fridgeo_category', ['slug'], unique=False)
    op.create_index('fridgeo_category_name_91d07872_like', 'fridgeo_category', ['name'], unique=False)
    op.create_index('fridgeo_category_name_91d07872', 'fridgeo_category', ['name'], unique=False)
